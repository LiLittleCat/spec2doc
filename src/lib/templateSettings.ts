...
const normalizeTemplateSettings = (
  ...
  apiTemplatePath: normalizePath(
    settings?.apiTemplatePath,
    DEFAULT_API_TEMPLATE_PLACEHOLDER,
  ),
  dbTemplatePath: normalizePath(
    settings?.dbTemplatePath,
...
export const subscribeTemplateSettings = (
  listener: (settings: TemplateSettings) => void,
) => {
  if (typeof window === "undefined") {
    return () => {};
  }
  ...
  window.addEventListener(
    TEMPLATE_SETTINGS_UPDATED_EVENT,
